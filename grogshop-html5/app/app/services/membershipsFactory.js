(function(){
    var $memberships = function($transformRequestToForm, $cookieStore, $http, appConstants){
        var factory = {};
        //LOAD MEMBERSHIPS
        factory.load = function(){
            return $http({
                method: 'GET',
                url: appConstants.server + appConstants.context + 'rest/members/'+ $cookieStore.get('user_id'),
        ...
        factory.join = function(club_id){
             return $http({
                method: 'POST',
                url: appConstants.server + appConstants.context + 'rest/members/join',
                headers: {'Content-Type': 'application/x-www-form-urlencoded', service_key: 'webkey:' + $cookieStore.get('email'), auth_token: $cookieStore.get('auth_token')},
                transformRequest: $transformRequestToForm.transformRequest,
        ...
        factory.cancel = function(club_id){
            return $http({
                method: 'POST',
                url: appConstants.server + appConstants.context + 'rest/members/cancel',
                headers: {'Content-Type': 'application/x-www-form-urlencoded', service_key: 'webkey:' + $cookieStore.get('email'), auth_token: $cookieStore.get('auth_token')},
                transformRequest: $transformRequestToForm.transformRequest,
                data: {club_id: club_id, user_id: $cookieStore.get('user_id')}
            })
    ...
    $memberships.$inject = ['$transformRequestToForm','$cookieStore', '$http', 'appConstants'];
